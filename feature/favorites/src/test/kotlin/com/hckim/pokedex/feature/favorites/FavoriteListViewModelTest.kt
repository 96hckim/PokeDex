...
class FavoriteListViewModelTest {
    ...
    private val getFavoritePokemonListUseCase: GetFavoritePokemonListUseCase = mockk()
    ...
    fun setUp() {
        ...
        every { getFavoritePokemonListUseCase() } returns flowOf()
    ...
    fun `ToggleFavorite 인텐트 전달 시 유스케이스를 호출해야 한다`() = runTest {
        ...
        // When
        viewModel.onIntent(FavoriteListUiIntent.ToggleFavorite(pokemon))
    ...
    @Test
    fun `ClickPokemon 인텐트 전달 시 상세 화면 이동 이펙트를 발생시켜야 한다`() = runTest {
        // Given
        val pokemon = Pokemon(id = 1, name = "bulbasaur", imageUrl = "", types = emptyList())

        // When & Then
        viewModel.effect.test {
            viewModel.onIntent(FavoriteListUiIntent.ClickPokemon(pokemon))
            val effect = awaitItem()
            assert(effect is FavoriteListUiEffect.NavigateToDetail && effect.pokemonName == pokemon.name)
        }
    }
}
